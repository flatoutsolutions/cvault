...
| M6  | Medium | convex/users/actions.ts:13 (org/orgMember sites deleted — see commit 4d9c55c)                                                                                                                                                  | Internal Clerk-webhook upsert mutations validate args as `v.any() as Validator<UserJSON>`. Even though the webhook handler verifies an svix signature first, the internal mutation has no contract enforcement — anything that calls it via `internal.users.actions.upsert` skips schema validation. The cast also violates the project's "no `as any` / no type-bypass" rule. Originally flagged at three sites; the `organizations` and `organizationMembers` domains were deleted in commit 4d9c55c, leaving only `users/actions.ts:13`. | Define a strict `v.object({...})` validator matching the subset of `UserJSON` the handler reads (id, first_name, last_name, image_url, email_addresses, primary_email_address_id). This both tightens types and protects against a future programming mistake that calls this internal mutation with mis-shaped data. (Pre-existing Blueprint code, not strictly cvault — but it's in the audited tree.)                                            |
...
- No `console.log`/`console.error` in any token-handling path leaks plaintext. `validateRequest.ts:16` logs only the verification error object (no token). `webhooks/clerk.ts:59` logs `event.type` only. `users/actions.ts:46` logs only Clerk IDs. (`organizations/actions.ts:46` was deleted with the org domain in commit 4d9c55c.)
...
- The single `v.any()` use is the Clerk-webhook upsert in `users/actions.ts:14` — see M6. (The `organizations` and `organizationMembers` domains were deleted in commit 4d9c55c, removing the other two sites originally flagged here.)
...
7. **Pre-existing Blueprint code** — `users/actions.ts`, `webhooks/clerk.ts`, `utils/validateRequest.ts` — these are Blueprint defaults; M6 flags the `v.any()` validator on the Clerk-webhook upsert, but this predates cvault and is shared with all Blueprint projects. (The `organizations/actions.ts` and `organizationMembers/actions.ts` files originally listed here were deleted in commit 4d9c55c.)
