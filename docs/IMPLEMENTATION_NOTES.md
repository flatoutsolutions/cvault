...
| **M6 — Clerk webhook `v.any()` validators**                        | Narrowed to `convex/users/actions.ts:14` only. The `organizations` and `organizationMembers` domains were deleted in commit 4d9c55c (perf-findings rec #5 + spec §2 single-tenant), removing two of the three original M6 sites. Remaining `users.actions.ts` upsert still uses `v.any() as Validator<UserJSON>` — pre-existing Blueprint code, deferred to a Blueprint-aware agent. |
