 ...
 *   1. Read CVAULT_*_URL or VITE/CLERK fallback env vars
 ...
 *      don't pick up baked URLs from a previous build
 *
...
describe('writeBuildInfo', () => {
  ...
  it('writes EMPTY_BUILD_DEFAULTS as three empty strings (the dev-reset shape)', () => {
    const target = join(tmp, 'buildInfo.ts')
...
describe('resolveBuildDefaultsFromEnv', () => {
  ...
  it('returns empty strings for any URL not present in env', () => {
