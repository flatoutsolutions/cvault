 ...
 *
 * The contract is intentionally narrow: ConvexError → formatted string,
 * everything else → `null` (caller falls through to the existing generic
 * display path so non-ConvexError behavior stays unchanged per the bug-fix
 ...
 *
 * Returns `null` when the value isn't a `ConvexError` — the caller is
 * expected to fall back to its existing display logic in that case.
 */
